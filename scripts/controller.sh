...
jobico::create_cluster() {
    ...
    # TLS
    if ! jobico::was_done "tls_certs"; then
...
jobico::create_nodes() {
    ...
    # TLS
    if ! jobico::was_done "add_tls"; then
    ...
    # Kubeconfig
    if ! jobico::was_done "add_kubeconfig"; then
        jobico::kubeconfig::gen_for_nodes
        NOT_DRY_RUN jobico::kubeconfig::deploy_to_nodes
        jobico::set_done "add_kubeconfig"
    fi
    # Deployment
    if ! jobico::was_done "add_deploy_nodes"; then
        NOT_DRY_RUN jobico::cluster::deploy_to_nodes
        jobico::set_done "add_deploy_nodes"
    fi
    # Routes
    if ! jobico::was_done "add_routes"; then
        NOT_DRY_RUN jobico::net::init
        #NOT_DRY_RUN jobico::net::add_routes
    ...
    # Merge DBs
    if ! jobico::was_done "add_merge_db"; then
        NOT_DRY_RUN jobico::dao::merge_dbs
        jobico::set_done "add_merge_db"
    fi
    # Lock
    if ! jobico::was_done "add_lock"; then
        NOT_DRY_RUN jobico::dao::cluster::lock
...
jobico::init_for_add() {
    ...
    if ! jobico::was_done "add_init"; then
...
jobico::local() {
    if ! jobico::was_done "locals"; then
...
jobico::install_all_addons() {
    local action=$1
    local base_dir=$2
    if ! jobico::was_done $action; then
