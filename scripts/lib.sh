...
jobico::kube::deps(){
  ...
  cp machines.txt ${WORK_DIR}
	cd ${WORK_DIR} && git clone --depth 1 https://github.com/kelseyhightower/kubernetes-the-hard-way.git \
		       && mv kubernetes-the-hard-way ${EXTRAS}
...
jobico::kube::generate(){
  ...
  jobico::kube::cluster::update_hostnamesi_file
...
jobico::kube::kubeconfig::gen_for_nodes(){
	for host in ${NODE_TBL[*]}; do
	  kubectl config set-cluster kubernetes-the-hard-way \
		--certificate-authority=c${WORK_DIR}/a.crt \
		--embed-certs=true \
...
jobico::kube::kubeconfig::gen_for_controlplane(){
  for host in ${COMPONENTS_CP_TBL[*]}; do
    kubectl config set-cluster kubernetes-the-hard-way \
    ...
    kubectl config set-credentials system:kube-proxy \
      --client-certificate=${WORK_DIR}/kube-proxy.crt \
      --client-key=${WORK_DIR}/kube-proxy.key \
      --embed-certs=true \
      --kubeconfig=${WORK_DIR}/kube-proxy.kubeconfig
...
jobico::kube::etcd::deploy_to_server(){
  scp ${DOWNLOADS_DIR}/etcd-v3.4.27-linux-amd64.tar.gz \ 
  ${EXTRAS_DIR}/units/etcd.service root@server:~/
