...
display_help_for_new() {
  ...
  echo "     --node-[0-9]"
  echo "            Custom configuration for the nodes, the values supported are:"
  echo "            * taints=[taint_name]"
  echo "            * cgroup=v2|v1"
  echo "            * size=std|small"
  echo ""
  echo "            Example:  k jobico new --nodes 1 --schedulable-server --node-0 taints=judge0,cgroup=v1,size=small"
  echo ""
  echo "     --gates [dir]"
